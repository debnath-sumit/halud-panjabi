...
export const storage = {
  async getContent() {
    const page = await list({ prefix: 'content/snapshots/', limit: 1000 });
    if (!page.blobs.length) return { intro: {}, shows: [], overrides: {} };
    const latest = page.blobs.sort((a, b) => b.pathname.localeCompare(a.pathname))[0];
    const response = await fetch(latest.url, { signal: AbortSignal.timeout(10_000) });
    if (!response.ok) throw new Error('Unable to read site content');
    const value = await response.json();
    return { intro: value.intro || {}, shows: Array.isArray(value.shows) ? value.shows : [], overrides: value.overrides || {} };
  },
  async saveContent(content) {
    await put(`content/snapshots/${Date.now()}-${randomUUID()}.json`, JSON.stringify(content), { access: 'public', contentType: 'application/json', addRandomSuffix: false });
  },
  async list() {
    ...
    const adjusted = records.map(item => content.overrides[item.id] ? { ...item, ...content.overrides[item.id] } : item);
    return applyMemberOrder(adjusted.sort((a, b) => b.createdAt - a.createdAt || a.id.localeCompare(b.id)), ids);
  },
  ...
  async putAsset(pathname, body, contentType) {
    return put(pathname, body, { access: 'public', contentType, addRandomSuffix: false });
  },
