...
Band members can be moved Earlier or Later in the manager, saving each change immediately. Saved order is shared by the public member grid and portrait lightbox. New members follow the saved line-up. Order snapshots use immutable `media/member-order/` paths to avoid stale CDN reads; old snapshots are retained. Reordering requires admin authentication and rejects stale member lists or duplicate IDs.
