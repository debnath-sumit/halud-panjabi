...
async function refresh() {
  ...
  for (const item of items) {
    ...
    if (item.kind !== 'clips') { const edit = document.createElement('button'); edit.className = 'quiet'; edit.textContent = 'Edit'; edit.addEventListener('click', async () => { if (item.kind === 'members') { const form = $('#member-form'); form.elements.id.value = item.id; form.elements.name.value = item.name; form.elements.role.value = item.role; form.elements.note.value = item.note; form.elements.photo.required = false; $('#member-form-title').textContent = 'Edit band member'; form.scrollIntoView({ behavior: 'smooth', block: 'start' }); return; } const next = prompt('Title', item.title); if (next === null || !next.trim()) return; if (item.kind === 'photos') { const picker = document.createElement('input'); picker.type = 'file'; picker.accept = 'image/jpeg,image/png,image/webp'; picker.click(); picker.addEventListener('change', async () => { const file = picker.files[0]; if (!file) return; try { notice('Uploading the replacement photo…'); const imageData = await preparePhoto(file); await saveContent({ method: 'PATCH', body: JSON.stringify({ kind: 'media-edit', id: item.id, title: next, imageData }) }); await refresh(); notice('Photo and title updated.', 'success'); } catch (error) { notice(error.message, 'error'); } }); return; } try { await saveContent({ method: 'PATCH', body: JSON.stringify({ kind: 'media-edit', id: item.id, title: next }) }); await refresh(); notice('Title updated.', 'success'); } catch (error) { notice(error.message, 'error'); } }); content.append(edit); }
